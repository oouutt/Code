...
{      			    
	for(a=0;a<16;a++)
	{
		if(e == 1) c = ~tfont16[b].Msk[a];
		else c = tfont16[b].Msk[a];
...
{      			    
	for(a=0;a<16;a++)
	{
		if(e == 1) c = ~tfont16[b].Msk[a+16];
		else c = tfont16[b].Msk[a+16];
		OLED_WR_Byte(c,OLED_DATA);
	}	
}
/********************************************
...
extern void OLED_ONONE_SHOW(u8 a,u8 b,u8 c,u8 d,u8 e);
extern void OLED_ONTOW_SHOW(u8 a,u8 b,u8 c,u8 d,u8 e);
	...
	for(m=0;m<Strnum;)//需要显示多少个汉子就在字库中检索多少次
	{
		for (k=0;k<HZnum;k++)//字库循环检索
		{
			...
			{ 	
				s = s + 2;
				OLED_Set_Pos(x,y);	
				OLED_ONONE_SHOW(t, k, data, adder,mode);
				OLED_Set_Pos(x,y+1);
				OLED_ONTOW_SHOW(t, k, data, adder,mode);
				x=x+16;				
				m++;
				break;
			}
