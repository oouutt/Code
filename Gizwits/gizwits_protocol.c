...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagmode)
    {
        info->event[info->num] = EVENT_mode;
        info->num++;
        dataPoints->valuemode = gizStandardDecompressionValue(mode_BYTEOFFSET,mode_BITOFFSET,mode_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagtoushi)
    {
        info->event[info->num] = EVENT_toushi;
        info->num++;
        dataPoints->valuetoushi = gizStandardDecompressionValue(toushi_BYTEOFFSET,toushi_BITOFFSET,toushi_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagshengyu)
    {
        info->event[info->num] = EVENT_shengyu;
        info->num++;
        dataPoints->valueshengyu = gizX2Y(shengyu_RATIO,  shengyu_ADDITION, issuedData->attrVals.valueshengyu); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuemode != cur->valuemode)
    {
        GIZWITS_LOG("valuemode Changed\n");
        ret = 1;
    }
    if(last->valuetoushi != cur->valuetoushi)
    {
        GIZWITS_LOG("valuetoushi Changed\n");
        ret = 1;
    }
    if(last->valueshengyu != cur->valueshengyu)
    {
        GIZWITS_LOG("valueshengyu Changed\n");
        ret = 1;
    }
    if(last->valuebuchong != cur->valuebuchong)
    {
        GIZWITS_LOG("valuebuchong Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    if(dataPoints->valuemode >= mode_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuemode Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(mode_BYTEOFFSET,mode_BITOFFSET,mode_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemode);  
    }
    if(dataPoints->valuetoushi >= toushi_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuetoushi Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(toushi_BYTEOFFSET,toushi_BITOFFSET,toushi_LEN,(uint8_t *)devStatusPtr,dataPoints->valuetoushi);  
    }
    if(dataPoints->valuebuchong >= buchong_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuebuchong Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(buchong_BYTEOFFSET,buchong_BITOFFSET,buchong_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebuchong);  
    }
    ...
    devStatusPtr->valueshengyu = gizY2X(shengyu_RATIO,  shengyu_ADDITION, dataPoints->valueshengyu); 
